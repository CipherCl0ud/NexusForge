...
import { Table2, Download, Loader2, AlertCircle, X, FileCheck2, UploadCloud, CheckCircle2, RotateCcw, ShieldCheck } from 'lucide-react';

const ACCEPT = '.xlsx';
const ENDPOINT = 'http://localhost:8000/api/excel-to-pdf';
...
export default function ExcelToPdf() {
  ...
  const addFiles = (fileList) => {
    const valid = Array.from(fileList).filter(f => f.name.toLowerCase().endsWith('.xlsx'));
    if (valid.length === 0) {
      setError('Please select valid .xlsx Excel files.');
      return;
    }
  ...
  const convert = async () => {
    if (files.length === 0) { setError('Please add at least one Excel file.'); return; }
    setProcessing(true);
    setError(null);
        ...
        <div className="border-b border-white/5 pb-4 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-white font-medium flex items-center gap-2 text-lg">
              <Table2 size={20} className="text-accent" /> Excel to PDF
            </h3>
            <p className="text-sm text-slate-400 mt-1">Transform one or more .xlsx Excel spreadsheets into clean, printable PDF files.</p>
          </div>
        ...
        {result ? (
          ...
          <>
            ...
            >
              ...
              <p className="text-xs text-slate-500">...or drop .xlsx files here</p>
            </div>
